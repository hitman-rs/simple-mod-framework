...
<div bind:this={titleElem}>
	<h1 class="text-6xl 2xl:text-7xl font-bold">
		...
		<sl-button
			...
			size="large"
			onclick={() => {
				const utterance = new SpeechSynthesisUtterance(`${m.UnrecognisedGameVersion()}. ${m.UnrecognisedGameVersionNotUpdatedDesc()}`)
				utterance.rate = 0.8
...
<p bind:this={firstElem} class="pt-4 2xl:pt-8 text-4xl 2xl:text-5xl font-bold text-green-300">{m.UnrecognisedGameVersionNotUpdatedDesc()}</p>
{#if titleElem && firstElem}
	{#each new Array(Math.floor(((visualViewport?.height || 0) - firstElem.offsetHeight - titleElem.offsetHeight - 100) / firstElem.offsetHeight)).keys() as idx}
		<p class="pt-4 2xl:pt-8 text-4xl 2xl:text-5xl font-bold {idx % 2 === 0 ? 'text-yellow-300' : 'text-green-300'}">{m.UnrecognisedGameVersionNotUpdatedDesc()}</p>
	{/each}
{/if}
